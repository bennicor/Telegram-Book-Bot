...
def get_book(book):
    ...
    search_URI = f"https://aldebaran.ru/pages/rmd_search_arts/"
    payload = {"q": book}
    page = requests.get(search_URI, params=payload).text
    bpage = BeautifulSoup(page, "html.parser")

    # Check if anything were found
    matches = bpage.find_all("p", string="В результате поиска ничего не найдено")
...
def get_more(link):
    ...
    result = [title, author, annotation.text, fixed_download if html else download]
    return result


if __name__ == "__main__":
    # get_book("обломов")
    get_more("https://aldebaran.ru/author/vitalius_vasiliyi/kniga_istorii_seksualnyih_oblomov/")
